...
```{r}
oilflow.hat = predict(oil.lmq, test.data1)
```
...
rf.model=train(oil.flow_barrels~power_megawatt, data=training.data1, method="rf",
               ntree=5000, trControl=ctrl, tuneGrid=data.frame(mtry = 1:8))
rf.model
oilflow.fit=predict(rf.model,training.data1)
mean(L(training.data1$oil.flow_barrels,oilflow.hat))

oilflow.hat=predict(rf.model,test.data1)
mean(L(test.data1$oil.flow_barrels,oilflow.hat))
```
...
I have created a table in Excel that I read into R which is presented in Table 2. This table completes the first part of the problem, finding all of the required data and information for each pipeline. I found this information by doing some quick research on the internet. I hope that everything is correct, some information was more difficult to find than others just from the way some companies had their information structured and publicly given out. 

To begin the main portion of the problem, I have already created a dataframe in R containing all of the information given from Genscape for each pipeline in Cushing. The head of this dataframe (labelled in my work as `prob2_data`) is in Table 3.

Our goal here is to use this data to create a model using the pipeline data to predict storage changes at Cushing. The information found in Table 2 could help check our model as well, since we now know the maximum amounts of oil that can be pumped in barrels per day, as well as whether or not that oil is being pumped in or out of Cushing. 
